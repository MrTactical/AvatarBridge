...
//
// ONLY the objects the bake GENERATES. A user's own object called "SPS
// Plug" is their name for their thing and stays theirs.
//
// WHAT IS DELIBERATELY NOT RENAMED: the pointer TYPE strings and the
// marker light RANGES. Those are not names, they are the wire, they are
// how a plug belonging to somebody who has never heard of this tool
// finds a socket on an avatar that came through it. Renaming
// SPSLL_Socket_Root would make every DPS, TPS and SPS plug on the
// platform blind to this avatar. Parameters are left alone too:
// ChilloutVR's profiles restore saved values by parameter NAME, so
// renaming a synced one silently empties everybody's saved settings.
//
// The hazard is entirely in the REPATHING. A clip addresses an object
// by path, so renaming a socket moves every descendant path with it,
// including the curves that switch that socket's marker lights on. A
// missed path does not error; the toggle simply stops. Hence: rewrite
// by PREFIX, not by exact match, and sweep afterwards to prove every
// path a clip still names resolves.
...
namespace AvatarBridge
{
    public static class YapsRename
    {
        ...
        // Generated by the bake, so this pass may name them. Ordered longest-first is
        ...
        public static void Run(BridgeContext ctx)
        {
            ...
            {
                // Loud, because the failure is invisible in game until somebody
                // notices a toggle doing nothing. And NAMED: a count alone once
                // fired at 86 and left nothing to diagnose from, because the paths
        ...
        static List<string> Unresolved(BridgeContext ctx, HashSet<string> deadBefore)
        {
            ...
            foreach (string path in AnimatedPaths(ctx).OrderBy(p => p, StringComparer.Ordinal))
            {
                ...
                // Resolved the way the animator resolves, not with Find: the
                // converted contact triggers under every socket are named after
                // their parameter, slashes and all, and Find called all
                // eighty-six of them dead while every one of them worked in game.
